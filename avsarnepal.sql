-- Avsar Nepal Database Schema
-- Drop database if exists and create new
DROP DATABASE IF EXISTS avsarnepal;
CREATE DATABASE avsarnepal CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE avsarnepal;
...
CREATE TABLE users (
    ...
    INDEX idx_career_centre (career_centre_id)
);
...
-- Job offers table (for employer-to-student direct job offers)
...
-- User career centre associations
CREATE TABLE user_career_centres (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    career_centre_id INT NOT NULL,
    joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_active BOOLEAN DEFAULT TRUE,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (career_centre_id) REFERENCES users(id) ON DELETE CASCADE,
    UNIQUE KEY unique_user_centre (user_id, career_centre_id),
    INDEX idx_user (user_id),
    INDEX idx_centre (career_centre_id),
    INDEX idx_active (is_active)
);
...
-- Add foreign key constraint for career_centre_id (self-referencing)
ALTER TABLE users 
ADD FOREIGN KEY (career_centre_id) REFERENCES users(id) ON DELETE SET NULL;
